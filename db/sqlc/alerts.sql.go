...
const appErrorSummarySince = `-- name: AppErrorSummarySince :many
SELECT app, COUNT(*) AS cnt, message AS sample, MAX(ts) AS last_ts
FROM app_logs WHERE level = 'ERROR' AND ts >= ?
GROUP BY app
`

type AppErrorSummarySinceRow struct {
	App    string      `json:"app"`
	Cnt    int64       `json:"cnt"`
	Sample string      `json:"sample"`
	LastTs interface{} `json:"last_ts"`
}
...
func (q *Queries) AppErrorSummarySince(ctx context.Context, ts time.Time) ([]AppErrorSummarySinceRow, error) {
	rows, err := q.db.QueryContext(ctx, appErrorSummarySince, ts)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AppErrorSummarySinceRow
	for rows.Next() {
		var i AppErrorSummarySinceRow
		if err := rows.Scan(
			&i.App,
			&i.Cnt,
			&i.Sample,
...
const deleteAlertLogsBeforeBatch = `-- name: DeleteAlertLogsBeforeBatch :execresult
DELETE FROM alert_log WHERE id IN (
    SELECT al.id FROM alert_log al WHERE al.created_at < ?1 ORDER BY al.id LIMIT ?2
)
`

type DeleteAlertLogsBeforeBatchParams struct {
	Cutoff    time.Time `json:"cutoff"`
	BatchSize int64     `json:"batch_size"`
}

func (q *Queries) DeleteAlertLogsBeforeBatch(ctx context.Context, arg DeleteAlertLogsBeforeBatchParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, deleteAlertLogsBeforeBatch, arg.Cutoff, arg.BatchSize)
}
